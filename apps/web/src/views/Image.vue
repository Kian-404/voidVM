    ...
    >
      <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <!-- 加载状态 -->
            <div v-if="loadingImageDetails" class="text-center py-5">
              <div class="spinner-border text-primary mb-3" role="status">
                <span class="visually-hidden">Loading...</span>
              </div>
              <p class="text-muted mb-0">加载镜像详情...</p>
            </div>
            ...
            <div v-else-if="imageDetails">
                ...
                <div class="card-body p-0">
                  <div class="list-group list-group-flush">
                    <div class="list-group-item d-flex justify-content-between align-items-start">
                      <div class="fw-medium text-muted" style="min-width: 80px">名称</div>
                    ...
                    <div class="list-group-item d-flex justify-content-between align-items-start">
                      <div class="fw-medium text-muted" style="min-width: 80px">类型</div>
                      <div class="flex-grow-1 ms-3 text-end">
                        <span
                          class="badge"
                          :class="imageDetails.type === 'iso' ? 'bg-success' : 'bg-info'"
                        >
                          {{
                    ...
                    <div class="list-group-item d-flex justify-content-between align-items-start">
                      <div class="fw-medium text-muted" style="min-width: 80px">路径</div>
                      <div class="flex-grow-1 ms-3 text-end">
                    ...
                    <div class="list-group-item d-flex justify-content-between align-items-start">
                      <div class="fw-medium text-muted" style="min-width: 80px">创建时间</div>
                      <div class="flex-grow-1 ms-3 text-end">
                        <span class="text-dark">{{ formatDate(imageDetails.created) }}</span>
                      </div>
                    </div>

                    <div class="list-group-item d-flex justify-content-between align-items-start">
                      <div class="fw-medium text-muted" style="min-width: 80px">修改时间</div>
                      <div class="flex-grow-1 ms-3 text-end">
                        <span class="text-dark">{{ formatDate(imageDetails.modified) }}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="card-body p-0">
                  <div class="list-group list-group-flush">
                    <div class="list-group-item d-flex justify-content-between align-items-center">
                      <div class="fw-medium text-muted">实际大小</div>
                      <div>
                        <span class="badge bg-success bg-opacity-10 text-success fs-6">
                          {{ formatSize(imageDetails.actualSize) }}
                        </span>
                      </div>
                    </div>

                    <div
                      v-if="imageDetails.virtualSize"
                      class="list-group-item d-flex justify-content-between align-items-center"
                    >
                      <div class="fw-medium text-muted">虚拟大小</div>
                      <div>
                        <span class="badge bg-info bg-opacity-10 text-info fs-6">
                          {{ formatSize(imageDetails.virtualSize) }}
                        </span>
                      </div>
                    </div>

                    <div
                      v-if="imageDetails.clusterSize"
                      class="list-group-item d-flex justify-content-between align-items-center"
                    >
                      <div class="fw-medium text-muted">簇大小</div>
                      <div>
                        <span class="badge bg-warning bg-opacity-10 text-warning fs-6">
                          {{ formatSize(imageDetails.clusterSize) }}
                        </span>
                      </div>
                    </div>

                    <div
                      v-if="imageDetails.backingFile"
                      class="list-group-item d-flex justify-content-between align-items-start"
                    >
                      <div class="fw-medium text-muted" style="min-width: 80px">基础镜像</div>
                      <div class="flex-grow-1 ms-3 text-end">
                        <code class="bg-light px-2 py-1 rounded small">{{
                          imageDetails.backingFile
                        }}</code>
                      </div>
                    </div>
                  </div>
              ...
              <!-- 进度条显示磁盘使用率 -->
              <div
                v-if="imageDetails.virtualSize && imageDetails.actualSize"
                class="card border-0 shadow-sm mb-4"
              >
                <div class="card-body">
                  <div class="d-flex justify-content-between align-items-center mb-2">
                    <span class="fw-medium">磁盘使用率</span>
                    <span class="text-muted small">
                      {{ Math.round((imageDetails.actualSize / imageDetails.virtualSize) * 100) }}%
                    </span>
                  </div>
                  ...
                  <div class="d-flex justify-content-between mt-1">
                    <small class="text-muted"
                      >已用: {{ formatSize(imageDetails.actualSize) }}</small
                    >
                    <small class="text-muted"
                      >总计: {{ formatSize(imageDetails.virtualSize) }}</small
                    >
                  </div>
                </div>
              </div>
              ...
              <!-- 技术详情（可折叠） -->
              <div v-if="imageDetails.fullInfo" class="card border-0 shadow-sm">
                <div class="card-header bg-light">
                  <h6 class="mb-0">
                    <button
                      class="btn btn-link p-0 text-decoration-none fw-medium d-flex align-items-center w-100"
                      type="button"
                ...
                <div id="technicalDetails" class="collapse">
                  <div class="card-body">
                    <pre
                      class="bg-dark text-light p-3 rounded-3 small mb-0"
            ...
            <!-- 错误状态 -->
            <div v-else-if="imageDetailsError" class="alert alert-danger d-flex align-items-center">
              <i class="bi bi-exclamation-triangle-fill me-2"></i>
              <div>{{ imageDetailsError }}</div>
            </div>
          </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <button
              ...
              @click="deleteImage"
              :disabled="(selectedImage && selectedImage.type === 'disk' && !confirmDiskDelete) ||
                deletingImage
                "
            >
