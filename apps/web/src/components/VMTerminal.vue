<template>
  <div
    ...
    :style="{ '--terminal-height': isFullscreen ? '80vh' : '400px' }"
  >
    <div
      class="card-header bg-primary text-white d-flex justify-content-between align-items-center"
    >
      <h5 class="mb-0"><i class="bi bi-terminal me-2"></i>{{ vm.name }} 终端</h5>
      <div class="d-flex">
    ...
    <div v-if="isFullscreen && showTerminal" class="fullscreen-hint">
      <div class="alert alert-info alert-dismissible fade show" role="alert">
        <i class="bi bi-info-circle me-2"></i>按 <kbd>ESC</kbd> 或 <kbd>F11</kbd> 退出全屏
...
<script setup lang="ts">
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (
      (event.key === 'F11' || (event.ctrlKey && event.shiftKey && event.key === 'F')) &&
      showTerminal.value
    ) {
